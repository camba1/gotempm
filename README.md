...
**Note:** At this time, starting multiple services in a row (which the startup scripts do) sometimes causes an error to be thrown. The details of the issue are described in [issue 11](https://bitbucket.org/Bolbeck/gotempm/issues/11/rpc-error-code-canceled-desc-grpc-the).
